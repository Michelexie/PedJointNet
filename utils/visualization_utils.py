...
def draw_bounding_box_on_image(image,
  ...
  try:
    font = ImageFont.truetype('arial.ttf', 18) #default is 24
  except IOError:
...
def visualize_boxes_and_labels_on_image_array(
  ...
  txt_filename = image_path.split('.jpg')[0]  # s10而不是s10.jpg
  # ouput = open('D:\\models\\research\\object_detection\\1216detection_txts\\' + txt_filename + ".txt", 'w')
  box_to_display_str_map = collections.defaultdict(list)
  # print('box_to_display_str_map',box_to_display_str_map) #defaultdict(<class 'list'>, {})
  box_to_color_map = collections.defaultdict(str)
  # print(box_to_color_map) #defaultdict(<class 'str'>, {})
...
#  box_to_keypoints_map = collections.defaultdict(list)
  ...
  # print('max_boxes_to_draw',max_boxes_to_draw)#20
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh: #选出大于min_score_thresh的box
      box = tuple(boxes[i].tolist()) #!!!这一步把100个框筛选成最终输出检测的个数
      # print('-----------------100-detected-box--------------',box)
     ...
     ######!
        txt_class_name = str(class_name)
        txt_confidence = str(float(scores[i]))
        # print('txt_class_name, txt_confidence:\n',txt_class_name, txt_confidence)
      ...
      txt_xmax = str(box[3])
      txt_ymax = str(box[2])
      # print('txt_ymin,txt_xmin,txt_ymax,txt_xmax:\n',txt_ymin,txt_xmin,txt_ymax,txt_xmax)

      # with open('D:\\models\\research\\object_detection\\1216detection_txts\\'+ txt_filename + ".txt", 'a',) as f:  # 打开test.txt   如果文件不存在，创建该文件。
      #
      #     f.writelines([txt_class_name,' ', txt_confidence, ' ', txt_xmin, ' ', txt_xmax, ' ', txt_ymin, ' ', txt_ymax, '\n'])
      # f.close()
...
#image_是预测出的bounding box（被截取后的）
    image_ = image[new_ymin:new_ymax,new_xmin:new_xmax] 
#
#     plt.imshow(image_)
#     plt.show()
# #
    PATH_TO_crop_DIR = r'D:\\models\\research\\object_detection\\test_cropped'
    image_ = Image.fromarray(np.uint8(image_))
    t+=1
    image_.save(os.path.join(PATH_TO_crop_DIR, (str(t)+'_')+os.path.basename(image_path)))
####################################################################################
  # Draw all boxes onto image.
  for box, color in box_to_color_map.items():
    # print ('box:',box)
    ymin, xmin, ymax, xmax = box
    # print('2nd',ymin, xmin, ymax, xmax)
    #读入ground truth的坐标
    # groundtruth_ymin, groundtruth_xmin, groundtruth_ymax, groundtruth_xmax = (0.3,0.4,0.7,0.6)
    #比较两者的iou
